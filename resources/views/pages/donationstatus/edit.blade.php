          ...
          <div class="col-12">
              <div class="card">
                  <div class="card-header">
                    <h3 class="card-title">Form Edit Status Donasi</h3>
                  </div>
                  <form action="{{ route('donationstatus.update', $item->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    <div class="card-body">
                      <div class="form-group">
                        <label for="name">Nama Status Donasi <span class="text-danger">*</span></label>
                        <input type="text" value="{{ old('name') ?? $item->name }}" class="form-control" id="name" name="name" placeholder="Enter Nama Status Donasi" required>
                      </div>
                      <div class="form-group">
